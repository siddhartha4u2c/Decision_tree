...
def plot_importance_figure(importances: pd.Series) -> plt.Figure:
    """Horizontal bar chart — avoids st.bar_chart / Altair (fragile on bleeding-edge Python)."""
    s = importances.sort_values(ascending=True)
    fig, ax = plt.subplots(figsize=(8, 3.8))
    ax.barh(s.index.astype(str), s.values, color="steelblue")
    ax.set_xlabel("Importance")
    ax.set_xlim(0, max(s.max() * 1.08, 0.01))
    fig.tight_layout()
    return fig
...
def main() -> None:
    ...
    with tab_importance:
        ...
        with i1:
            ...
            fig_ig = plot_importance_figure(imp_g)
            st.pyplot(fig_ig)
            plt.close(fig_ig)
        with i2:
            ...
            fig_ie = plot_importance_figure(imp_e)
            st.pyplot(fig_ie)
            plt.close(fig_ie)
